"""
Sentence Encoder for Duration Predictor

テキストからセンテンスエンベディングを生成するモジュール

アーキテクチャ (tts.json の dp.sentence_encoder):
1. Text Embedder: Unicode IDs → 64次元
2. ConvNeXt: 6層の前処理
3. Attention Encoder: 2層のTransformer (LARoPE使用)
...
class TextEmbedder(nn.Module):
    ...
    Unicode Text IDsを64次元のエンベディングに変換
    """
...
class SentenceEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Output Projection
        proj_out_idim: int = 64,
        proj_out_odim: int = 64,
    ):
        """
        引数:
            vocab_size: 語彙サイズ
    ...
    def forward(
        ...
        戻り値:
            センテンスエンベディング [batch, odim, seq_len]
        """
        # 1. Text Embedder
